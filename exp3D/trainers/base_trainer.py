...
class BaseTrainer():
    ...
    def run(self):
        ...
        if self.cfg.get('eval_model') is not None:
            print('Load model:', self.cfg['eval_model'])
        ...
        elif self.cfg.get('resume_model') is not None:
            print('Load model:', self.cfg['resume_model'])
            model_spec = torch.load(self.cfg['resume_model'], map_location='cpu')['model']
            self.make_model(model_spec, load_sd=True)
            self.train()
        else:
            self.make_model()
    ...
    def make_model(self, model_spec=None, load_sd=False):
        ...
        if self.distributed:
            ...
            model.cuda()
            model_ddp = DistributedDataParallel(model, device_ids=[self.rank], find_unused_parameters=True)
        else:
            model.cuda()
            model_ddp = model
        self.model = model
