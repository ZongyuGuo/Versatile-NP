...
def poses_to_rays(poses, image_h, image_w, focal):
    ...
    y, x = torch.meshgrid(torch.arange(image_w, device=device), torch.arange(image_h, device=device)) # h w
...
def get_coord(rays_o, rays_d, near, far, points_per_ray, rand):
    """
        Args:
...
def rendering(raw, rays_o, z_vals):
    B = raw.shape[0]
...
def volume_rendering(nerf, rays_o, rays_d, near, far, points_per_ray, use_viewdirs, rand):
    """
        ...
        Returns:
            pred: (b ... 3)
    """
    pts_flat, z_vals = get_coord(rays_o, rays_d, near, far, points_per_ray, rand)
    ...
    rgb_map = rendering(aw, rays_o, z_vals)
    return rgb_map
