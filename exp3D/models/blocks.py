...
class ToMixLogistic(nn.Module):
    def __init__(self, in_channel, out_channel, style_dim):
    # def __init__(self, in_channel, out_channel, style_dim, upsample=False, blur_kernel=[1, 3, 3, 1]):
        super().__init__()
        # self.upsample = upsample
        # if upsample:
        #     self.upsample = Upsample(blur_kernel)
...
class ToMixLogisticNoCond(nn.Module):
...
class SinActivationBias(nn.Module):
    def __init__(self, out_channel):
        super(SinActivationBias, self).__init__()
        self.bias = nn.Parameter(torch.zeros(1, 1, out_channel))

    def forward(self, x):
        return torch.sin(x + self.bias)


class LFF(nn.Module):
    def __init__(self, hidden_size, ):
        super(LFF, self).__init__()
        self.ffm = ConLinear(1, hidden_size, is_first=True)
